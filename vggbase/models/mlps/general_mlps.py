...
def build_mlp_from_config(
    ...
    :param  mlp_configs: A Dict type containing the hyper-parameters for definition.
        It should contains:
            input_n_features: Integar
            output_n_features: Integar
            hidden_layers_n_features: List[int], with length N - 1
    ...
    """
    input_n_features = mlp_configs["input_n_features"]
    output_n_features = mlp_configs["output_n_features"]

    hidden_layers_n_features = mlp_configs["hidden_layers_n_features"]
    hidden_n = len(hidden_layers_n_features)
    ...
    hidden_layers_n_features.append(output_n_features)
    for hid_id, hid_dim in enumerate(hidden_layers_n_features):
        layer_input_n_features = (
            input_n_features if hid_id == 0 else hidden_layers_n_features[hid_id - 1]
        )
...
class Model:
    ...
    def get(
        ...
        **kwargs: Dict[str, Union[int, List[Union[str, None, dict]]]],
        ...
        """Get the desired MLP model with required hyper-parameters (input_n_features)."""
        ...
        if model_name == "simclr_projection_mlp":
            ...
            return build_mlp_from_config(
                dict(
                    output_n_features=output_n_features,
                    input_n_features=input_n_features,
                    hidden_layers_n_features=[projection_hidden_dim],
        ...
        if model_name == "simsiam_prediction_mlp":
            ...
            return build_mlp_from_config(
                dict(
                    output_n_features=output_n_features,
                    input_n_features=input_n_features,
                    hidden_layers_n_features=[prediction_hidden_dim],
        ...
        if model_name == "multilayer":
            return build_mlp_from_config(
                dict(
                    output_n_features=output_n_features,
                    input_n_features=input_n_features,
                    hidden_layers_n_features=[1024, 512, 256, 128],
